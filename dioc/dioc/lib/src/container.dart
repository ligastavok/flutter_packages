...
class Container {
  ...
  void register<T>(Creator<T> creator, {String? name, InjectMode defaultMode = InjectMode.unspecified}) {
  ...
  T? singleton<T>({String? name, String? creator}) {
    final Factory<T>? result = this._getFactory<T>();
  ...
  /// used.
  T? get<T>({String? name, String? creator, InjectMode mode = InjectMode.unspecified}) {
    final Factory<T>? result = this._getFactory<T>();

    return result?.get(name: name, creator: creator, mode: mode);
  }

  T? safeGet<T>({String? name, String? creator, InjectMode mode = InjectMode.unspecified}) {
  ...
  T? call<T>({String? name, String? creator, InjectMode mode = InjectMode.unspecified}) {
  ...
  bool hasAll(List<Type> types, {String? creator}) {
  ...
  Factory<T> _getOrCreateFactory<T>(InjectMode defaultMode) {
    final Factory<dynamic> res = this._factories.putIfAbsent(
          T,
          () => Factory<T>(this, defaultMode: defaultMode),
        );

    return res as Factory<T>;
  }

  Factory<T>? _getFactory<T>({bool safeGet = false}) {
    Factory<dynamic>? factory = this._factories[T];
...
class Factory<T> {
  ...
  T call({String? name, String? creator, InjectMode mode = InjectMode.unspecified}) {
    return this.get(name: name, creator: creator, mode: mode);
  }

  T get({String? name, String? creator, InjectMode mode = InjectMode.unspecified}) {
